...
contract BeastIDO is Ownable {
    ...
    function contribute(uint256 _amount) external {
        ...
        ContributeData storage userContributeData = contributedPerUser[_msgSender()];
    ...
    function claimTokens() external {
        require(block.timestamp > endDate, "IDO not finished");
        ContributeData storage userContributedData = contributedPerUser[_msgSender()];
        ...
        if (userRequiredAmount <= availableSellAmount) {
            ...
            feeAmount = userContributedAmount.mul(fee).div(10000);
        ...
        // Contract does not have enough balance
            ...
            feeAmount = payingAmount.mul(fee).div(10000);
            ownerFunds = payingAmount.sub(feeAmount);
        }
